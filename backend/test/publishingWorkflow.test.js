...
const { createPolicy, actorFrom } = require('../src/services/governedWorkflow');
const config = require('../src/config/publishingWorkflow');
...
const readyRecord = { status: 'review_ready', payload: {"brief":"launch","brandRulesVersion":"v3","channels":["search"],"draftVersion":"7","disclosureRequired":true,"modelOutputTrusted":false}, evidence };
...
test('provisioned reviewer with attestation can approve a ready record', () => {
  const decision = policy.authorizeTransition(readyRecord, 'approved', { id: '2', role: 'brand_reviewer' }, { attestation: 'I reviewed the authoritative evidence.', evidenceIds });
...
test('approval attestation must cite every accepted required source', () => {
  const decision = policy.authorizeTransition(readyRecord, 'approved', { id: '2', role: 'brand_reviewer' }, { attestation: 'I reviewed the authoritative evidence.', evidenceIds: ['source-0'] });
