...
function createPolicy(config) {
  ...
  function authorizeTransition(record, targetStatus, actor, approval) {
    ...
    if (config.approvalStatuses.includes(targetStatus)) {
      ...
      if (requiredSources.some((item) => !selected.has(item.externalId))) {
        return { ok: false, code: 'APPROVAL_EVIDENCE_MISMATCH' };
      }
    }
    return { ok: true };
  }
  ...
  function authorizeEvidence(item, actor) {
    if (!config.requiredEvidence.includes(item.type)) {
      return { ok: false, status: 400, code: 'UNSUPPORTED_EVIDENCE_TYPE' };
    }
    if (item.authoritative && item.status === 'accepted') {
      if (!SHA256.test(item.checksum) || !item.observedAt || Number.isNaN(Date.parse(item.observedAt))) {
